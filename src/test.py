...
class TileModel(object):
    ...
    def build_rules(self):
        ...
        img = self._translated_image.translated_image
        for y in range(len(img) - (len(self._patterns[0]) - 1)):
            for x in range(len(img[0]) - (len(self._patterns[0][0]) - 1)):
                pattern = self._get_pattern((x, y), (len(self._patterns[0][0]), len(self._patterns[0])))
                for direction in directions.Directions:
                    if direction.is_valid((x, y), (len(img[0]), len(img)), (len(pattern[0]), len(pattern))):
